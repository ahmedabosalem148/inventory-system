...
class RealDataSeeder extends Seeder
{
    ...
    private function getNotificationsForRole(User $user, array $templates): array
    {
        ...
        // All users get a mix of notifications
        // Vary the count based on user email to add diversity
        if (str_contains($user->email, 'admin') || str_contains($user->email, 'manager')) {
        ...
        } elseif (str_contains($user->email, 'warehouse')) {
            // Warehouse staff focus on inventory and orders
            $notifications = array_merge(
                $this->generateFromTemplate($templates[0], 5, $now), // Low stock
        ...
        } elseif (str_contains($user->email, 'sales')) {
        ...
        } elseif (str_contains($user->email, 'inventory')) {
            ...
            $notifications = array_merge(
                $this->generateFromTemplate($templates[2], 3, $now), // Orders
                $this->generateFromTemplate($templates[5], 3, $now)  // General
            );
        }
