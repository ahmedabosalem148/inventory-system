...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'http://localhost/api/v1/notifications/unread-count');
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'http://localhost/api/v1/notifications/recent');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HTTPHEADER, [
    'Authorization: Bearer ' . $token,
    'Accept: application/json',
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'http://localhost/api/v1/notifications?per_page=5');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HTTPHEADER, [
    'Authorization: Bearer ' . $token,
    'Accept: application/json',
...
$data = json_decode($response, true);
if (isset($data['data'])) {
    echo "Total: {$data['meta']['total']}, Page: {$data['meta']['current_page']}/{$data['meta']['last_page']}\n";
