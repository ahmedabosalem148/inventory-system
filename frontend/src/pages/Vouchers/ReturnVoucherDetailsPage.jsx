...
const ReturnVoucherDetailsPage = () => {
  const { id } = useParams();
  ...
  const fetchVoucherDetails = async () => {
    ...
    try {
      // Get voucher details
      const voucherResponse = await apiClient.get(`/return-vouchers/${id}`);
      ...
      console.error('Failed to fetch return voucher details:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="pt-16 lg:mr-64 p-6 min-h-screen">
          <div className="flex items-center justify-center h-64">
            <div className="text-red-500">فاتورة المرتجع غير موجودة</div>
          </div>
        </main>
      </div>
    );
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-4 md:mb-6 gap-4">
            <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 sm:space-x-reverse">
              <Button
                variant="outline"
                size="sm"
                onClick={() => navigate('/return-vouchers')}
                className="flex items-center space-x-2 space-x-reverse w-fit"
              >
                <ArrowRight className="w-4 h-4" />
                <span className="hidden sm:inline">العودة للمرتجعات</span>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center space-x-3 space-x-reverse">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">فاتورة مرتجع</p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2 space-x-reverse">
                        ...
                        <span className="text-gray-600">تاريخ المرتجع:</span>
                        <span className="font-medium">{formatDate(voucher.return_date)}</span>
                      </div>
                      ...
                      {voucher.original_voucher_number && (
                        <div className="flex items-center space-x-2 space-x-reverse">
                          <FileText className="w-4 h-4 text-gray-400" />
                          <span className="text-gray-600">الفاتورة الأصلية:</span>
                          <span className="font-medium">{voucher.original_voucher_number}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {voucher.reason && (
                        <div className="flex items-start space-x-2 space-x-reverse">
                          ...
                          <span className="text-gray-600">سبب المرتجع:</span>
                          <span className="font-medium">{voucher.reason}</span>
                        </div>
                      )}
                        ...
                        {voucher.items && voucher.items.map((item, index) => (
                          <tr key={index} className="hover:bg-gray-50">
                            ...
                            <td className="px-3 md:px-6 py-4 text-xs md:text-sm text-gray-900 whitespace-nowrap">
                              {item.returned_quantity} {item.product?.unit || ''}
                            </td>
                            <td className="px-3 md:px-6 py-4 text-xs md:text-sm text-gray-900 whitespace-nowrap">
            ...
            <div className="space-y-4 md:space-y-6 order-1 xl:order-2">
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="border-t pt-3">
                      <div className="flex justify-between items-center text-base md:text-lg font-semibold text-orange-600">
              ...
              <Card>
                <div className="p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-sm md:text-base text-gray-600">مبلغ المرتجع:</span>
                      <span className="font-medium text-orange-600 text-sm md:text-base">{formatCurrency(voucher.total_amount)}</span>
                    </div>
                    
                    <div className="flex justify-between items-center">
                      <span className="text-sm md:text-base text-gray-600">المسترد:</span>
                    ...
                    {remaining === 0 && (
                      <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                        <div className="flex items-center space-x-2 space-x-reverse text-green-700">
                          ...
                          <span className="text-sm font-medium">تم الاسترداد بالكامل</span>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <Card>
                  <div className="p-6">
                    <h3 className="text-lg font-semibold mb-4 flex items-center space-x-2 space-x-reverse">
                      ...
                      <span>تاريخ المبالغ المستردة</span>
                    </h3>
...
export default ReturnVoucherDetailsPage;